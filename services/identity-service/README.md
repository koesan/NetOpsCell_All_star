...
**Faz 8:** İki kademeli OTP teslimatı — müşteri e-posta girip SMTP yapılandırılmışsa kod
gerçekten e-postaya gönderilir (`src/auth/email.service.ts`), aksi halde web'de gösterilir;
her iki durumda da kod `otp_codes` tablosunda müşteri kaydıyla (`userId`) ilişkili saklanır
(bkz. kök README "Müşteri OTP Doğrulaması"). Ayrıca: refresh token reuse-detection'da canlı
testte bulunan kritik bir açık (`IsNull()` operatörü eksikliği) düzeltildi — bkz. kök README
"Güvenlik" bölümü ve `src/auth/auth.service.spec.ts`.
...
| POST | `/api/v1/auth/register` | Müşteri kaydı + OTP tetikleme (kod yalnızca sunucu logunda) |
